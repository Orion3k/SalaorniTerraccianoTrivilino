...
%\usepackage{geometry} 
    ...
    \begin{itemize}
        \item A main server (possibly distributed to guarantee better reliability and scalability).
        \item A database to store collected data.
    \end{itemize}
    ...
    The system, to provide its services, exploits the devices of the users which use third parties applications based on Data4Help. 
    
    \paragraph{AutomatedSOS:} The system is based on a Client-Server architecture, moreover it has to communicate with Data4Help and the ambulance dispatcher. It is composed by:
    \begin{itemize}
        \item A server to manage the users' data and handle the emergency status.
    \end{itemize}
    The service can be provided to the user through his/her mobile devices (Smartphone, smartwatch, smartwears, etc.). The device must respect the constraints that Data4Help requires in order to 
    ...
    \end{itemize}
    Data4Help furthermore gives to the third parties a set of client-side APIs that provide an interface in order to connect their own application with Data4Help services.
    
    \paragraph{AutomatedSOS:} the system needs to manage the communication with Data4Help, to analyze the data and to communicate with the ambulance dispatcher and the communication between the app and the server. The interfaces needed are:
    ...
    Data4Help collect data from the users through its Client-side APIs. The third parties has to integrate the Data4Help API in their own app to access to their services. Data4Help acquires data from the users' devices and then provides them to the ThirdParties, managing the requests and the permissions. 
